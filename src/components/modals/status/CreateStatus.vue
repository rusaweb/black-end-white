<template>
  <BaseModal title="Создать статус" id="create-status">
    <template #content>
...
const submit = async () => {
  ...
  try {
    await storeStatus.createStatus(state)
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#create-status')
})
defineComponent({name: 'ModalCreateStatus'})
</script>
