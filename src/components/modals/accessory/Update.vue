<template>
  <BaseModal title="Редактировать акксессуар" id="update-accessory">
    <template #content>
      <div class="block-content block-content-full">
        <div v-if="!load" class="row items-push">
          <div class="col-12">
            ...
            <div class="row">
              <div class="col-md-3 col-4 mb-2" style="height: 100px;" v-for="(item, key) in state.images" :key="key">
...
const accessory = computed(() => storeAccessory.accessory)
...
const submit = async () => {
  ...
  try {
    await storeAccessory.updateAccessory(state)
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#update-accessory')
  document.getElementById('update-accessory').addEventListener('show.bs.modal', async (event) => {
    load.value = true
    state.id = event.relatedTarget.dataset.bsId
    await storeAccessory.getAccessory(state.id)
    state.images = accessory.value.images
    state.name = accessory.value.name
    state.description = accessory.value.name
    state.price = accessory.value.price
    load.value = false
  })
})
const deleteItemImage = async (item) => {
  try {
    await storeAccessory.deleteImageAccessory(accessory.value.id, item.id, async () => {
      await storeAccessory.getAccessory(accessory.value.id)
      state.images = accessory.value.images
    })
  }catch (e) {
    console.log(e)
...
const cropperImage = async () => {
  cropper.value.getCroppedCanvas({
    ...
    await storeAccessory.uploadImageAccessory(accessory.value.id, file, async () => {
      await storeAccessory.getAccessory(accessory.value.id)
      state.images = accessory.value.images
    })
  });
}
defineComponent({name: 'ModalUpdateAccessory'})
</script>