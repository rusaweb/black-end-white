<template>
  <BaseModal title="Добавить адрес" id="create-address">
        ...
        <form action="" class="form-block items-push row">
          ...
          <div class="col-lg-6 fw-normal" v-if="!state.location">
            ...
            <button
                ...
                :class="{ 'is-invalid': v$.location.$errors.length}"
                data-bs-target="#map"
    ...
    <template #footer>
      <button class="btn btn-success me-2" @click.prevent="submit">Сохранить</button>
...
const submit = async () => {
  ...
  try {
    await storeUser.createAddress(state)
    state.address_line = ''
...
defineComponent({name: "ModalCreateAddressProfile"})
</script>