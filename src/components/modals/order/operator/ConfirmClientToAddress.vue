...
const submit = async () => {
  ...
  try {
    await storeOperator.updateDeliverToAddress({...state, ...router.currentRoute.value.params})
    await resetState()
    await cancelOrder()
    v$.value.$reset()
...
const cancelOrder = () => {
  storeOperator.cancelOrder(router.currentRoute.value.params.id)
}
onMounted(() => {
  ...
  document.getElementById('confirm-client-to-address').addEventListener('show.bs.modal', async () => {
    state.user_id = storeOperator.order.user_id
    await storeOperatorUser.getUserById(state.user_id)
  })
})
defineComponent({name: 'ModalConfirmClientToAddress'})
</script>
