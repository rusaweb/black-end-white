<template>
  <BaseModal title="Редактирование адреса" id="update-deliver-to-address">
    <template #content>
        ...
        <ul class="list-group">
          ...
          <li class="list-group-item d-flex justify-content-between align-items-center" v-if="deliverDelivery">
            <div class="btn-group w-100">
              ...
              <button class="btn btn-sm w-100" :class="deliverIsAddress ? 'btn-primary' : 'btn-secondary'"
                      @click.prevent="deliverIsAddress = true">Мои адреса
              </button>
            </div>
          </li>
          ...
          <li class="list-group-item d-flex justify-content-between align-items-center" v-if="deliverDelivery && !deliverIsAddress">
            ...
            <div class="col-6">
              <template v-if="!state.deliver_to_address.location">
                <button
                    ...
                    data-bs-back="#update-deliver-to-address"
                    :data-bs-index="2"
                    @click.prevent
...
const submit = async () => {
  ...
  try {
    await storeUser.updateOrderDeliverToAddress({...state, id: router.currentRoute.value.params.id})
    await resetState()
    v$.value.$reset()
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#update-deliver-to-address')
  let modalInfo = document.getElementById('update-deliver-to-address')
  modalInfo.addEventListener('show.bs.modal', ()=> {
    if (storeUser.order.deliver_to_address.type === 'department'){
      deliverDelivery.value = false
    }else {
      deliverDelivery.value = true
      state.deliver_to_address = storeUser.order.deliver_to_address
    }
  })
})
defineComponent({name: 'ModalUpdateDeliverToAddress'})
</script>
