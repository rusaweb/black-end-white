<template>
  <BaseModal title="Добавить промо-код" id="create-promo-code">
    <template #content>
...
const date = (new Date((new Date(Date.now() - (new Date().getTimezoneOffset() * 60000)).setSeconds(0, 0)))).toISOString().split(':')
date.pop()
const min = date.join(':');
let state = reactive({
  id: "",
...
const submit = async () => {
  ...
  try {
    ...
    await storePromoCode.createPromoCode(state)
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#create-promo-code')
})
defineComponent({name: 'ModalCreatePromoCode'})
</script>
