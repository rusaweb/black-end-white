<template>
  <BaseModal title="Добавить обработку" id="create-additional-processing">
    <template #content>
...
const submit = async () => {
  ...
  try {
    ...
    await storeAdditionalProcessing.createAdditionalProcessing(state)
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#create-additional-processing')
})
defineComponent({name: 'ModalCreateAdditionalProcessing'})
</script>
