<template>
  <BaseModal title="Добавить тип" id="create-order-item-type">
    <template #content>
      <div class="block-content block-content-full">
        <div v-if="!load" class="row items-push">
          ...
          <div class="input-group" v-if="categories_children?.content">
...
const submit = async () => {
  ...
  try {
    ...
    state.category_id = state.category_children.id
    await storeOrderItemType.createOrderItemType(state)
...
onMounted(() => {
  ...
  modal.value = new window.bootstrap.Modal('#create-order-item-type')
  load.value = false
})
...
defineComponent({name: 'ModalCreateOrderItemType'})
</script>
