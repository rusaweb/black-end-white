<template>
  <BaseModal title="Добавить этап" id="create-order-item-step">
    <template #content>
...
const submit = async () => {
  ...
  try {
    await storeOperatorOrderItem.createOrderItemStep(state)
...
onMounted(() => {
  modal.value = new window.bootstrap.Modal('#create-order-item-step')
})
defineComponent({name: 'ModalCreateOrderItemStep'})
</script>
