      ...
      <div class="block-content block-content-full" v-if="issues?.content.length">
        <div class="table-responsive">
          <table class="table table-bordered table-striped table-vcenter">
            ...
            <tbody>
            <tr style="cursor:pointer;" v-for="item in issues?.content" :key="item.id" @click="$router.push({name: 'order-issue-user-detail', params: {order_id: order?.id, id: item.id}})">
...
const orderIssueStore = useOrderIssuesUserStore()
...
defineComponent({name: 'OrderDetailUserOrderIssueList'})
</script>
