<template>
  <BasePageHeading title="Список дефектов" subtitle="На этой странице отображаются теги дефектов"/>
  <div class="content">
    <BaseBlock title="Список дефектов" ref="tableBlock">
      <template #options>
        <div class="space-x-1">
          <button class="btn btn-block-option" data-bs-target="#create-tag" data-bs-toggle="modal" data-bs-code="defects">
...
tagsStore.getTagsDefects(router.currentRoute.value.query);


onBeforeRouteUpdate((to, from, next) => {
  tagsStore.getTagsDefects(to.query);
  next();
});
...
const tags = computed(() => {
  ...
  return tagsStore.tags_defects;
});

const initTag = async () => {
  await tagsStore.getTagsDefects(router.currentRoute.value.query);
}
