          ...
          <div class="table-responsive">
            <table class="table table-bordered table-striped table-vcenter">
              ...
              <tr v-for="order in orders?.content" :key="order.id">
                ...
                <td class="text-end" v-if="filter === 'unmappedOnly'">
                  <button class="btn btn-alt-primary" @click.prevent="accept(order.id)">Принять</button>
                </td>
                <td class="text-end" v-if="filter === 'mappedToSelfOnly'">
                  <button class="btn btn-alt-primary" @click.prevent="refuse(order.id)">Отказаться</button>
                </td>
              </tr>
              </tbody>
            </table>
...
watch(filter, (value, oldValue) => {
  ...
  storeOrderCourier.getOrderListWaiting(router.currentRoute.value.query)
})
...
onBeforeRouteUpdate((to, from, next) => {
  storeOrderCourier.getOrderListWaiting(to.query)
  next()
})
const accept = async (id) => {
  await storeOrderCourier.handleOrderDelivery(id)
  await storeOrderCourier.getOrderListWaiting(router.currentRoute.value.query)
}
const refuse = async (id) => {
  await storeOrderCourier.mishandleOrderDelivery(id)
  await storeOrderCourier.getOrderListWaiting(router.currentRoute.value.query)
}
</script>
<style scoped>
