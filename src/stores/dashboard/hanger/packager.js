...
export const useHangerPackagerStore = defineStore('hanger-packager', () => {
    ...
    const getHanger = async (id) => {
        loading.value = true
        try {
            hanger.value = (await axios.get('/api/v1/hanger/packager/' + id)).data
        } catch (err) {
            throw new Error(err)
        }finally {
            loading.value = false
    ...
    const getHangerByCode = async (code) => {
        loading.value = true
        try {
            hanger.value = (await axios.get('/api/v1/hanger/packager/code/' + code)).data
        } catch (err) {
            throw new Error(err)
        }finally {
            loading.value = false
    ...
    const getHangers = async (args) => {
        loading.value = true
        try {
            hangers.value = (await axios.get('/api/v1/hanger/packager/?' + new URLSearchParams(args).toString())).data
        } catch (err) {
            throw new Error(err)
        }finally {
            loading.value = false
