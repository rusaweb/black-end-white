...
export const useUserAdministratorStore = defineStore('user-administrator', () => {
    ...
    const getUserDiscountCard = async (id) => {
        loading.value = true
        try {
            user.value = (await axios.get('/api/v1/user/administrator/discount-card/' + id)).data
        } catch (err) {
            throw new Error(err)
        }finally {
            loading.value = false
    ...
    const getUsersList = async (args) => {
        ...
        users.value = null
        try {
            users.value = (await axios.get('/api/v1/user/administrator?onlyWithoutRoles=true&' + new URLSearchParams(args).toString())).data
        } catch (err) {
            throw new Error(err)
        }finally {
            loading.value = false
